...
export class CardDetector {
  ...
  detectCards(imageData: ImageData): DetectedCard[] {
    ...
    const testCards = this.generateTestCards(imageData.width, imageData.height);
    cards.push(...testCards);
    
    const contours = this.findContours(imageData);
    
    for (const contour of contours) {
      if (this.isRectangular(contour) && this.isCardSized(contour, imageData.width, imageData.height)) {
  ...
  private generateTestCards(width: number, height: number): DetectedCard[] {
    const cards: DetectedCard[] = [];
    
    const cardWidth = Math.min(width * 0.25, 150);
    ...
    for (let i = 0; i < 3; i++) {
      ...
      cards.push({
        ...
        corners: [
          { x: x - cardWidth/2, y: y - cardHeight/2 },
          { x: x + cardWidth/2, y: y - cardHeight/2 },
          { x: x + cardWidth/2, y: y + cardHeight/2 },
          { x: x - cardWidth/2, y: y + cardHeight/2 }
        ],
  ...
  private traceContours(edges: boolean[], width: number, height: number): Array<{x: number; y: number}[]> {
    const contours: Array<{x: number; y: number}[]> = [];
  ...
  private approximatePolygon(contour: Array<{x: number; y: number}>): Array<{x: number; y: number}> {
  ...
  private isCardSized(contour: Array<{x: number; y: number}>, imageWidth: number, imageHeight: number): boolean {
    const bounds = this.getBounds(contour);
  ...
  private getBounds(contour: Array<{x: number; y: number}>): {x: number; y: number; width: number; height: number} {
  ...
  correctPerspective(imageData: ImageData, corners: Array<{x: number; y: number}>): ImageData {
  ...
  private applyTransform(transform: number[][], point: {x: number; y: number}): {x: number; y: number} {
    const [h] = transform;
    const x = point.x;
    const y = point.y;
  ...
  private findConnectedComponents(imageData: ImageData): Array<{x: number; y: number}[]> {
    const { data, width, height } = imageData;
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        if (binary[idx] && !visited[idx]) {
          const component = this.floodFill(binary, visited, x, y, width, height);
